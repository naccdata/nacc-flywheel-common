...
def add_group_permission(group: Group, user_id: str, access: AccessLevel) -> None:
    ...
    existing_permissions = [perm for perm in group.permissions if perm.id == user_id]
    if not existing_permissions:
        group.add_permission(AccessPermission(id=user_id, access=access))
        return
    group.update_permission(user_id, AccessPermission(id=None, access=access))


def add_group_permissions(
    *, group: Group, user_ids: List[str], access: AccessLevel
) -> None:
